...
export const metadata = buildMetadata({
  ...
  description:
    "Read the latest articles and insights on ERP solutions, Microsoft Dynamics 365, and business process optimization from the experts at DAX.",
  canonicalPath: "/blog",
});
...
// ✅ Match Next's generated App Router types: searchParams may be a Promise
type PageProps = {
...
async function getPosts(
  ...
  try {
    const res = await fetch(
      `https://blog.daxsws.com/wp-json/wp/v2/posts?per_page=${perPage}&page=${page}&orderby=date&order=desc&status=publish&_embed=1`,
      { next: { revalidate: 600 } }
    );
  ...
  if (totalPages <= maxPagesToShow) {
    ...
    pages.push(
      <PaginationItem key={1}>
        <PaginationLink href={`/blog?page=1`} isActive={1 === currentPage}>
          1
        </PaginationLink>
      </PaginationItem>
    );
...
export default async function BlogPage({ searchParams }: PageProps) {
  const { page } = await searchParams; // ✅ await because it's a Promise
  const currentPage = parseInt(page || "1", 10);

  const { posts, totalPages } = await getPosts(currentPage);
  const fallbackImage = PlaceHolderImages.find((img) => img.id === "blog-fallback");

  // If user requests a page > 1 and WordPress returns empty, treat as 404
    ...
    <div className="bg-background">
      ...
      <section className="py-16 md:py-24">
        <div className="container mx-auto px-4">
                ...
                return (
                  ...
                  >
                    ...
                    <CardContent className="p-6 flex flex-col flex-grow">
                      ...
                      <div
                        className="text-muted-foreground flex-grow line-clamp-4"
          ...
          {totalPages > 1 && (
            <div className="mt-16">{renderPagination(currentPage, totalPages)}</div>
          )}
        </div>
      </section>
    </div>
